...
local function load_upnext()
    ...
    if string.find(url, ".mkv") ~= nil
    then
	    url = mp.get_property("filename")
	    url = "https://www.youtube.com/watch?v="..string.match(url, "^[^-]+")
		mp.osd_message(url)
	end
...
local function on_file_start(_)
    ...
    if string.find(url, "youtu") ~= nil then
        ...
        if s ~= nil then
            ...
            local s, e = string.find(url, "youtu.be/[^#?!&]+")
            if s ~= nil then
                video_id = string.sub(url, s + 9, e)
            end
        end
